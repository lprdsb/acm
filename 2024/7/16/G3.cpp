...
int main() {
    ...
    // for(auto [l, r, p] : v1) cout << l << " " << r << " " << p << endl;
    sort(v1.begin(), v1.end(), [](const Runs &x, const Runs &y) {return x.l == y.l ? (x.r == y.r ? x.p < y.p : x.r < y.r) : x.l < y.l;});
    For(i, 0, v1.size() - 1) {
        int l = v1[i].l, r = v1[i].r, p = v1[i].p;
        if(!i || l != v1[i - 1].l || r != v1[i - 1].r) {
            for(int kp = 2 * p; kp <= r - l + 1; kp += 2 * p) {
                For(i, l, l + p - 1) if(i + kp - 1 <= r) vec[sa[1].to[i]].pb(kp);
            }
        }
    }
    ...
    For(i, 1, n[1]) {
        ...
        for(auto j : vec[i]) if(j > now && j > sa[1].height[i]) num[j] = 0;
