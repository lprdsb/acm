...
void mian() {
    For(i, 0, 3) scanf("%lf%lf", &v[i].x, &v[i].y);
    // v[1] = v[1] - v[0];
    // v[3] = v[3] - v[2];
    Line l1 = Line(v[0], v[1]), l2 = Line(v[2], v[3]);
    // cout << "asd" << endl;
    if(fabs(l1.v ^ l2.v) <= eps) {
        if(fabs((v[2] - v[0]) ^ (v[1] - v[0])) <= eps) {
            if(chk(v[0], v[2], v[3]) || chk(v[1], v[2], v[3]) || chk(v[2], v[0], v[1]) || chk(v[3], v[0], v[1])) puts("intersect");
            else puts("parallel");
        }
        else puts("parallel");
        return;
    ...
    Vector tmp = l1 & l2;
    if(chk(tmp, v[0], v[1]) && chk(tmp, v[2], v[3])) {
        puts("intersect");
    }
    else puts("neither");
}
