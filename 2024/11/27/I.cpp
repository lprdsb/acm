...
int chk(Line l) {
    vector<Vector> vec;
    For(i, 1, n) if((a[i] - l.x) ^ l.v) vec.pb(a[i]);
    For(i, 2, (int)vec.size() - 1) if((vec[i] - vec[0]) ^ (vec[1] - vec[0])) return 0;
    return 1;
}
...
int main() {
    ...
    if((a[2] - a[1]) ^ (a[3] - a[1])) {
        ...
        else if(chk(Line(a[1], a[3]))) {
            puts("YES");
        }
        else if(chk(Line(a[2], a[3]))) {
            puts("Yes");
        }
        else puts("NO");
    }
    else if(chk(Line(a[1], a[2]))) puts("YES");
    else puts("NO");
    return 0;
}
