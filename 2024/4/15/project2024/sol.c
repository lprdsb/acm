...
Edge *Hash_add(Hash *self, int key) {
    Edge *res;
    if(res = Hash_find(&code_hsh, key)) return res;
    int kk = key % Hash_P;
...
void Hash_INIT(Hash *self) {
    memset(self->hd, 0, sizeof self->hd);
    self->key_tot = 0;
}
void Hash_init(Hash *self) {
    while(self->key_tot) self->hd[self->hsh_key[self->key_tot--]] = 0;
}
...
char *get_malloc(int len) { return malloc(len); }

int code_tot = 0;
typedef struct Code {
    int fun_tot, fun_key[105], fun_len[105], main_ind, id, len;
    char *s, *s_fun[maxm + 5];
} Code;
Code code[maxm + 5];
...
void Code_init(Code *self, char *s, int len) {
    ...
    For(i, 1, len) {
        if(!sum) {
            if(self->fun_tot) {
                ...
                self->fun_len[self->fun_tot] = now - 1;
            ...
            while(s[i] != ')') i++; i++;
            while(s[i] != '{') i++;
            if(key == 655907298) self->main_ind = self->fun_tot;
    ...
    For(i, 1, self->fun_tot) {
        ...
        For(j, 1, self->fun_len[i]) {
            if(!is_txt(ss[j])) {
                ...
                else if(Hash_find(&hsh, key) || digit) pre = now;
                else now = pre;
    ...
    self->len = len1;
    // For(i, 1, self->fun_tot) printf("%s\n", self->s_fun[i] + 1);
    // printf("%s\n", self->s + 1);
}

int str_to_num(char *s, int len) {
    int res = 0;
    For(i, 1, len) res = 10 * res + s[i] - '0';
    return res;
}
...
int main() {
    ...
    code_tot = 1;
    fgets(_s, maxn, file);
    if(!fgets(pos, maxn, file)) return 0;
    len = strlen(pos) - 1;
    code[code_tot].id = str_to_num(_s, len);
    pos = _s + 1;
    len = 0;
    int tot = 0;
    while(fgets(pos, maxn, file)) {
        tot++;
        len += strlen(pos) - 1;
        _s[len + 1] = '\0';
        if(*pos == '\f') {
    ...
    For(i, 1, code_tot) if(!vis[i]) {
        For(j, i + 1, code_tot) if(!vis[j]) {
            int tem = get_f(code[i].s, code[i].len, code[j].s, code[j].len);
            if(i == 1 && j == 2) printf("%.10lf\n", 1 - (DD)tem / max(&code[i].len, &code[j].len));
        ...
        if(as[i][0]) {
            printf("%d ", code[i].id);
            For(j, 1, as[i][0]) printf("%d ", code[as[i][j]].id);
            puts("");
        }
    }
}
