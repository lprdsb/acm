...
cst int P1 = 998244353, P2 = 1000000007, B1 = 1331, B2 = 1331;
...
int dep[maxn + 5], fa[maxn + 5], ok[maxn + 5];
...
int sz[maxn + 5];
Z f[maxn + 5];
int pri[maxn + 5], vis[maxn + 5], cc = 0;
void dfs1(int u, int pre){
    ...
    for(auto v : vec) f[u] = f[u] * bas[sz[v] + 1] + f[v];
    f[u] = f[u] * Z{sz[u] + 1};
    // if(u == 2) cout << f[u].v1.x << endl;
}
...
int main(){
    ...
    For(i, 2, maxn){
        if(!vis[i]) pri[cc++] = i;
        for(int j = 0; j < cc && pri[j] * i <= maxn; ++j){
            vis[i * pri[j]] = 1;
            if(i % pri[j] == 0) break;
        }
    }
    For(i, 1, maxn) bas[i] = Z{pri[i % cc]};
    read(T);
    while(T--){
        read(n); read(m);
        For(i, 1, m){
