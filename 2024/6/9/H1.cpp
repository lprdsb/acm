...
vector<int> con;
DD get_r(int p1, int p2, int p3) {
    Geometry::Vector v1 = poi[con[p2]] - poi[con[p1]], v2 = poi[con[p3]] - poi[con[p1]];
    DD c = v1 * v2 / v1.len() / v2.len(), s = sqrt(1 - c * c);
    return (poi[con[p3]] - poi[con[p2]]).len() / s;
}
Geometry::Vector get_cir(int p1, int p2, int p3) {
    Geometry::Vector md = poi[con[p1]] + poi[con[p2]];
    md.x /= 2; md.y /= 2;
    Geometry::Line l1; l1.x = md; l1.v = ~(poi[con[p1]] - poi[con[p2]]);
    md = (poi[con[p1]] + poi[con[p3]]);
    md.x /= 2; md.y /= 2;
    Geometry::Line l2; l2.x = md; l2.v = ~(poi[con[p1]] - poi[con[p3]]);
    Geometry::Vector cir = l1 & l2;
    return cir;
}
Z sol(int l, int r) {
    if(l + 1 == r) return 1;
    int mx = -1;
    Geometry::Vector mxc = { -1, -1 };
    For(i, l + 1, r - 1) {
        Geometry::Vector c = get_cir(l, r, i);
        if(mx == -1 || Geometry::dcmp((mxc - poi[con[l]]) ^ (c - poi[con[l]])) < 0) {
            mxc = c;
            mx = i;
        }
    }
    DD lr = (poi[con[l]] - mxc).len2();
    // cout << l << " " << r << endl;
    For(i, r + 1, l - 1 + len) if(Geometry::dcmp((poi[con[i]] - mxc).len2() - lr) > 0) return 0;
...
void mian() {
    ...
    For(i, 0, con.size() - 1) con.pb(con[i]);
    printf("%d\n", (sol(p[0], p[1]) * sol(p[1], p[0] + len) * C(len - 2, p[1] - p[0] - 1)).x);
}
