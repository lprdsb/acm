...
struct Poly : public std::vector<MInt<P>> {
    ...
    constexpr std::vector<Value> eval(std::vector<Value> x) const {
        ...
        std::function<void(int, int, int)> build = [&](int p, int l, int r) {
            ...
            // std::cout << l << " asd " << r << std::endl;
        };
        build(1, 0, n);
        for(auto i : q[1]) std::cout << i.x << " "; std::cout << std::endl;
        std::function<void(int, int, int, const Poly &)> work = [&](int p, int l, int r, const Poly &num) {
            std::cout << l << " " << r << std::endl;
            for(auto i : num) std::cout << i.x << " "; std::cout << std::endl;
            if (r - l == 1) {
                if (l < int(ans.size())) {
                    ans[l] = num[0];
                }
...
int main() {
    ...
    for(int i = 0; i < m; ++i) std::cin >> g[i].x;
    // cout << "asd" << endl;
    // for(auto i : f) cout << i.x << " "; cout << endl;
    std::vector<Z> res = f.eval(g);
