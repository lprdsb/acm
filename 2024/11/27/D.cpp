...
void mian() {
    Poly F, G;
    scanf("%s", s + 1); n = strlen(s + 1); F.resize(n);
    // cout << n << endl;
    For(i, 1, n) F[n - i] = s[i] - '0';
    scanf("%s", s + 1); m = strlen(s + 1); G.resize(m);
    For(i, 1, m) G[m - i] = s[i] - '0';
    F *= G;
    // cout << F.size() << endl;
    int fl = 0;
    For(i, 0, (int)F.size() - 2) F[i + 1].x += F[i].x / 10, F[i].x %= 10;
    Rof(i, F.size() - 1, 0) {
        if(fl || F[i].x) {
            fl = 1;
            // cout << i << endl;
