...
namespace Geometry {
    ...
    namespace HalfPlaneCross {
        ...
        DD get_cro_s() {
            For(i, 1, r - l) {
                cro[i] = a[dq[l + i - 1]] & a[dq[l + i]];
                if(dcmp(a[dq[l + i - 1]].v ^ a[dq[l + i]].v) == 0) return 0;
            }
            cro[r - l + 1] = a[dq[r]] & a[dq[l]];
            if(dcmp(a[dq[r]].v ^ a[dq[l]].v) == 0) return 0;
            // For(i, 1, r - l + 1) cout << cro[i].x << " " << cro[i].y << endl;
...
void mian() {
    ...
    For(i, 1, n) scanf("%lf%lf", &v[i].x, &v[i].y);
    Convex::init(v, n);
    Convex::mian();
    n = Convex::r - 1;
    For(i, 1, n) c[i] = Line(Convex::a[Convex::dq[i]], Convex::a[Convex::dq[i + 1]]);


    For(i, 1, m) scanf("%lf%lf", &v[i].x, &v[i].y);
    Convex::init(v, m);
    Convex::mian();
    m = Convex::r - 1;
    For(i, 1, m) c[n + i] = Line(Convex::a[Convex::dq[i]], Convex::a[Convex::dq[i + 1]]);

    HalfPlaneCross::init(c, n + m);
    if(HalfPlaneCross::mian()) puts("not empty");
    else puts("empty");
    // printf("%.1Lf\n", Convex::mian());
}
