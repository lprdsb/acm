...
int main() {
    ...
    // read(n); read(k);
    Poly F = Poly{ 1, -1 } *Poly{ 1, -2, 1, -1 }.inv(maxn), G{ 1, -1 };
    G = G.inv(maxn);
    G = G.shift(3);
    G[1] += 1;

    Z res = 0;
    auto GG = G.pfp(maxn, k - 2);
    For(i, 0, n - 4) res += F[i] * GG[n - 4 - i];
    GG *= G; For(i, 0, n - 2) res += F[i] * GG[n - 2 - i] * 2;
    GG *= G; For(i, 0, n) res += F[i] * GG[n - i];
    cout << res.x << endl;
    return 0;
}
