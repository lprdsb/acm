...
DD get_val(DD x, DD y){
    return sqrt(x * x + y * y);
}

DD get_simp(DD x, DD l, DD r){
    return (r - l) * (get_val(x, l) + get_val(x, r) + 4 * get_val(x, (l + r) / 2)) / 6;  
}

DD simp(DD x, DD l, DD r, DD now){
    DD mid = (l + r) / 2, teml = get_simp(x, l, mid), temr = get_simp(x, mid, r);
    if(fabs(now - teml - temr) <= eeps) return teml + temr;
    return simp(x, l, mid, teml) + simp(x, mid, r, temr);
}

DD get_simp1(DD l, DD r, DD t){
    return (r - l) * (simp(l, 0, t, get_simp(l, 0, t)) + simp(r, 0, t, get_simp(r, 0, t)) + 4 * simp((l + r) / 2, 0, t, get_simp((l + r) / 2, 0, t))) / 6;
}

DD simp1(DD l, DD r, DD t, DD now){
    DD mid = (l + r) / 2, teml = get_simp1(l, mid, t), temr = get_simp1(mid, r, t);
    if(fabs(now - teml - temr) <= eeps) return teml + temr;
    return simp1(l, mid, t, teml) + simp1(mid, r, t, temr);
}
