...
int main() {
    ...
    For(i, 1, n) {
        For(o, 1, k) {
            if(i >= 3) f[i][o] += f[i - 3][o - 1];
            f[i][o] += f[i - 1][o - 1] - (i >= 2 ? f[i - 2][o - 1] : 0);
        }
        if(i >= 3) f[i][k] += f[i - 3][k];
        f[i][k] += f[i - 1][k] - (i >= 2 ? f[i - 2][k] : 0);

        if(i == n - 4) res += f[i][k - 2] + f[i][k - 1] + f[i][k];
        if(i == n - 2) res += 2 * (f[i][k - 1] + f[i][k]);
        if(i == n) res += f[i][k];
        // if(i == n) cout << f[i][k].x << endl;
        For(o, 0, k) f[i][o] += f[i - 1][o];
    }
    printf("%d\n", res.x);
}