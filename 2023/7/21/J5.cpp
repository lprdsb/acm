...
struct Poly : vector<Z>{
    ...
    void ntt(Z gi){
        int lim = (*this).size();
        For(i, 0, lim - 1) if(rev[i] < i){
            Z tem = (*this)[rev[i]];
            (*this)[rev[i]] = (*this)[i];
            (*this)[i] = tem;
        }
        for(int i = 2; i <= lim; i <<= 1){
            vector<Z> w{1, fp(gi, (P - 1) / i)};
            int ii = i >> 1;
            For(j, 2, ii - 1) w.pb(w[1] * w[j - 1]);
            for(int j = 0; j < lim; j += i){
                For(p, j, j + ii - 1){
                    Z tem1 = (*this)[p], tem2 = w[p - j] * (*this)[p + ii];
                    (*this)[p] = tem1 + tem2;
                    (*this)[p + ii] = tem1 - tem2;
                }
            }
        ...
        Z tem = ((Z)lim).inv();
        if(gi == ig) For(i, 0, lim - 1) (*this)[i] *= tem;
    }
    ...
    inline Poly& operator *= (Poly _f){
        ...
        _f.resize(lim);
        this->ntt(g); _f.ntt(g);
        For(i, 0, this->size() - 1) cout << (*this)[i].x << " "; cout << endl;
